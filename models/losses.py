...
import numpy as np
from models.cyclegan_networks import init_net
...
class SpatialCorrelativeLoss(nn.Module):
    ...
    def create_kan(self, feat, layer):
        """
        create the 1*1 conv filter to select the features for a specific task
        :param feat: extracted features from a pretrained VGG or encoder for the similarity and dissimilarity map
        :param layer: different layers use different filter
        ...
        input_nc = feat.size(1)
        output_nc = max(32, input_nc // 4)
        kan = nn.Sequential(*[KAN(
            layers_hidden=[input_nc, self.output_nc, self.output_nc],
            grid_size=5,
            spline_order=3,
            scale_noise=0.1,
            scale_base=1.0,
            scale_spline=1.0,
            base_activation=torch.nn.LeakyReLU,  # torch.nn.SiLU
            grid_eps=0.02,
            grid_range=[-1, 1]
        ),
            KAN(
                layers_hidden=[output_nc, self.output_nc, self.output_nc],
                grid_size=5,
                spline_order=3,
                scale_noise=0.1,
                scale_base=1.0,
        ...
        kan.to(feat.device)
        setattr(self, 'conv_%d' % layer, kan)
        init_net(kan, self.init_type, self.init_gain, self.gpu_ids)
